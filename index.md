...
But let's study two solutions in this plot to compare their performance: solution S00066 (```max_features```: 5, ```min_sample_split```: 30) and solution S00004 (```max_features```: 2, ```min_sample_split```: 40). I have linked to the filtered state of this webpages with only these two solutions [here]() . We see that solution S00066 does better on *every* objective than solution S00004. By that reasoning, there would never be a reason to pick solution S00004 if all we cared about were these four objectives. Commonly, we say that solution S00066 "dominates" solution S00004. In order for a solution to "dominate" another, it needs to perform the same or better on all objectives and strictly better on at least one. Continuing this logic, we only really care about the *non*dominated solutions (which is the converse of dominated). So we apply a nondominated sort to this set and re-plot:
